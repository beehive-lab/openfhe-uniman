 ...
 * ring Dimension       = 8192  (is set internally by OpenFHE)
 * gpu blocks           = 8     (should be set manually at approx-switch-crt-basis.cu)
...
int main() {
    ...
    cudaUtils.initialize(16, 512, 2, 8192, 2, 2, 3);
    #endif
    ...
    // Output results
    std::cout << "\nResults of 1 homomorphic multiplication (1 * 1) : " << plaintextMultResult << std::endl;
